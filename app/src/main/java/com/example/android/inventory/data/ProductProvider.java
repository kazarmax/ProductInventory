...
public class ProductProvider extends ContentProvider {
    ...
    public String getType(Uri uri) {
        ...
        switch (match) {
            ...
            default:
                throw new IllegalArgumentException("Cannot define MIME type for " + uri);
        }
    }
    ...
    private Uri insertProduct(Uri uri, ContentValues values) {
        ...
        // Check that content values are provided and not null
    ...
    private int updateProduct(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        // Check that ContentValues are provided
        checkContentValues(values);
    ...
    /**
     * Private method for checking content values before inserting or updating products in the database
     */
    private void checkContentValues(ContentValues values) {

        // Check that ContentValues contain the name
        ...
        // Check that ContentValues contain the quantity
        if (values.containsKey(ProductEntry.COLUMN_PRODUCT_QUANTITY)) {
            // Check that the quantity is not negative
            int quantity = values.getAsInteger(ProductEntry.COLUMN_PRODUCT_QUANTITY);
            if (quantity < 0) {
                throw new IllegalArgumentException("Products's quantity cannot be negative");
            }
        }

        // Check that ContentValues contain the price
        ...
        // Check that ContentValues contain the supplier name
        ...
        // Check that ContentValues contain the supplier email
        if (values.containsKey(ProductEntry.COLUMN_PRODUCT_SUPPLIER_EMAIL)) {
            ...
            if (TextUtils.isEmpty(supplierEmail)) {
                throw new IllegalArgumentException("Product supplier requires a email");
            }
        }

        // Check that ContentValues contain the image
        if (values.containsKey(ProductEntry.COLUMN_PRODUCT_IMAGE)) {
            // Check that the product image is provided
            byte[] productImage = values.getAsByteArray(ProductEntry.COLUMN_PRODUCT_IMAGE);
